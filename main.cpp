...
int main(int argc, char** argv) {
	...
	for(io.F0=(io.I-io.J0).abs(); io.F0< io.I+io.J0+_frac<>(1); io.F0++) {
	  ...
	  std::cout  << std::setw(10) << "\t<H>_M1/A="+io.E_M1_divA().show();
	  if (!io.A0_isempty)
	    std::cout  << std::setw(10)  << "\t<H>_M1\u2243" <<  io.E_M1_divA().val()*io.A0;
	  std::cout << std::endl;
	}
	...
	for(io.F0=(io.I-io.J0).abs(); io.F0< io.I+io.J0+_frac<>(1); io.F0++) {
	  std::cout << "F="<< io.F0.show();
	  std::cout  << std::setw(10) << "\t<H>_E2/B="+io.E_E2_divB().show();
	  if (!io.B0_isempty)
	    std::cout  << std::setw(10)  << "\t<H>_E2\u2243" <<  io.E_E2_divB().val()*io.B0;
	  std::cout << std::endl;
	}
	/***********/
